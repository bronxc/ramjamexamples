...
frame2:
	cmpi.b	#$fd,$dff006	; Siamo alla linea 253?
	bne.s	frame2		; Se non ancora, non andare avanti
frame3:
	cmpi.b	#$fc,$dff006	; Siamo alla linea 252?
	bne.s	frame3
frame4:
	cmpi.b	#$fb,$dff006	; Siamo alla linea 251?
	bne.s	frame4

	btst	#2,$dff016	; se il tasto destro e' premuto salta
	beq.s	NonMuovere	; la routine dello scroll, bloccandolo

	bsr.s	MuoviCopper	; Routine dei moduli
...
; Con questa routine aggiungiamo o sottraiamo 40 ai registri dei moduli per
; accorciare la figura. Ho mantenuto le label dell'esempio precedente per
; risparmiare tempo.

MuoviCopper:
	TST.B	SuGiu		; Dobbiamo salire o scendere?
	beq.w	VAIGIU
	tst.w	MOD1		; Siamo al valore normale del modulo? (ZERO)
	beq.s	MettiGiu	; se si, dobbiamo aumentare il valore
	sub.w	#40,MOD1	; sottraiamo 40, ossia 1 linea, facendo
				; scorrere in BASSO la figura (ingrandendola)
	sub.w	#40,MOD2	; sottraiamo 40 al modulo 2
	rts

MettiGiu:
	clr.b	SuGiu		; Azzerando SuGiu, al TST.B SuGiu il BEQ
	rts			; fara' saltare alla routine VAIGIU
...
Per fare una cosa piu' "pulita" avrei dovuto mettere un wait sotto la figura
che mi togliesse di mezzo le "impurita" che si vedono in basso, ossia i byte
in memoria dopo la figura e i bitplanes che "spuntano" sotto il primo e il
secondo. Ma lo scopo principale e' quello di spiegare la funzione dei moduli.
La routine tra l'altro viene eseguita una volta ogni 4 fotogrammi per
rallentarla.
